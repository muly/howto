...
func (o *GetStudentByIDURL) Build() (*url.URL, error) {
	...
	var _path = "/api/students/{id}"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetStudentByIDURL")
	}
...
func (o *GetStudentByIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetStudentByIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetStudentByIDURL")
	}
